...
# 读取邻接链表并转化为字典
filename = 'Dgraph_results_150_average.txt'  # 文件路径
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if (u == edge[1] and w == v) or (u == w and v == edge[1]):
                r = tuple([u,v])
                rWeight = weight
    if l != tuple([-1,-1]) and r != tuple([-1,-1]) and lWeight != 0 and rWeight != 0:
        res += 1
    # index +=1
# and minWeight == max(max(lWeight, rWeight), minWeight)
# print(res / t)
# print(W_Delta / l1_sampler.total_weight / (n - 2))
res = res * l1_sampler.total_weight * (n - 2) / t / 3
with open("Dgraph_results_150_average_ranRes.txt",'w') as f:
    f.write(f"{res}\n")
print(res)
print(datetime.datetime.today())
