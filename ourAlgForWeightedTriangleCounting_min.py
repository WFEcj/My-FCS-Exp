...
# 读取邻接链表并转化为字典
filename = 'Dgraph_results_100_min_30.txt'  # 文件路径
...
delta = 0.01
n = 100
t = math.ceil(27 / epsilon**2 * 0.3 * (n - 2)  * math.log(2 / delta, 2))
print(t)
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if (u == edge[1] and w == v) or (u == w and v == edge[1]):
                r = tuple([u,v])
                rWeight = weight
    if l != tuple([-1,-1]) and r != tuple([-1,-1]) and lWeight != 0 and rWeight != 0 and minWeight == min(min(lWeight, rWeight), minWeight):
        res += 1
    # index +=1
...
with open("Dgraph_results_100_min_ranRes_30_3.txt",'w') as f:
    f.write(f"{res}\n")
print(res)
print(datetime.datetime.today())
